---
name: auth-security
description: "멍샵 프로젝트의 인증/보안 구현 표준. HttpOnly 쿠키 기반 JWT, 401 자동 갱신, FOUC 방지, 소셜 로그인 패턴을 다룬다. TRIGGER when: 로그인·로그아웃 기능을 구현할 때, JWT 토큰 저장 방식을 결정할 때, 인증이 필요한 페이지를 보호할 때, 401 응답 처리 또는 토큰 갱신 로직을 작성할 때, OAuth 소셜 로그인을 연동할 때, 인증 초기화(새로고침 후 로그인 상태 복원) 로직을 구현할 때. Do NOT use for: 인증과 무관한 일반 API 호출이나 UI 컴포넌트 작업."
