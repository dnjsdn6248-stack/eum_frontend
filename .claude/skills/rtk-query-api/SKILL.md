---
name: rtk-query-api
description: "멍샵 RTK Query API 레이어 구현 표준. 단일 createApi에 injectEndpoints로 도메인을 분리하는 구조, providesTags·invalidatesTags 캐시 무효화 전략, withReauth 401 자동 갱신 래퍼 적용 규칙, store.js 등록 절차를 정의한다. TRIGGER when: 새 API 엔드포인트를 추가하거나 기존 엔드포인트를 수정할 때, query·mutation의 캐시 태그 전략을 결정할 때, RTK Query 훅을 새로 생성하거나 도메인 슬라이스를 분리할 때, API 호출에 인증 토큰 갱신 로직을 적용할 때. Do NOT use for: Mock 환경 전용 엔드포인트 작업은 mock-architecture 스킬을 사용."
