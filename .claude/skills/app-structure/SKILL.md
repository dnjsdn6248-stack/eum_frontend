---
name: app-structure
description: "멍샵 Feature-based Design 디렉토리 구조, 라우팅 트리(MyPageLayout 중첩 포함), 컴포넌트 계층 역할, Redux store 구조를 정의한다. TRIGGER when: 새 페이지나 컴포넌트를 추가할 때, 새 도메인 폴더를 생성할 때, App.jsx 라우트 트리를 수정할 때, Layout·MyPageLayout 구조를 변경할 때, store.js에 새 슬라이스를 등록할 때. Do NOT use for: 컴포넌트 내부 로직이나 스타일 수정 등 구조 변경이 없는 작업."
